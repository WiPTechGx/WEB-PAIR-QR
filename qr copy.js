import express from 'express';
import fs from 'fs-extra';
import pino from 'pino';
import QRCode from 'qrcode';
import {
    makeWASocket,
    useMultiFileAuthState,
    DisconnectReason,
    fetchLatestBaileysVersion,
    delay,
    Browsers
} from '@whiskeysockets/baileys';
import { upload } from './mega.js';
import path from 'path';
import { fileURLToPath } from 'url';
import os from 'os';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const router = express.Router();

const SESSIONS_ROOT = process.env.VERCEL
    ? path.join(os.tmpdir(), 'sessions')
    : path.join(__dirname, 'sessions');

const MESSAGE = `*SESSION GENERATED SUCCESSFULLY* ✅

*Made with love by pgwiz* 🥀
profile: https://pgwiz.cloud`;

// Track active connections
const activeConnections = new Map();

// ============================================
// QR CODE GENERATION ENDPOINT
// ============================================
router.get('/', async (req, res) => {
    let customId = req.query.sessionId || '';
    customId = customId.replace(/[^a-zA-Z0-9]/g, '');
    const randomID = Math.random().toString(36).substring(2, 6);
    const sessionId = customId ? `pgwiz-${customId}` : `pgwiz-${randomID}`;

    const dirs = path.join(SESSIONS_ROOT, sessionId);

    if (!fs.existsSync(SESSIONS_ROOT)) {
        await fs.mkdir(SESSIONS_ROOT, { recursive: true });
    }

    async function initiateSession() {
        if (!fs.existsSync(dirs)) await fs.mkdir(dirs, { recursive: true });

        const { state, saveCreds } = await useMultiFileAuthState(dirs);

        try {
            const { version } = await fetchLatestBaileysVersion();
            console.log(`[${sessionId}] Baileys version: ${version.join('.')}`);

            let qrGenerated = false;
            let responseSent = false;
            let credsSent = false;
            let timeoutId = null;

            const sock = makeWASocket({
                version,
                logger: pino({ level: 'silent' }),
                printQRInTerminal: false,
                browser: Browsers.ubuntu('Chrome'),
                auth: state,
                syncFullHistory: false,
                markOnlineOnConnect: false,
            });

            sock.ev.on('creds.update', saveCreds);

            sock.ev.on('connection.update', async (update) => {
                const { connection, lastDisconnect, qr } = update;

                console.log(`[${sessionId}] Connection: ${connection}`);

                // Generate QR if provided
                if (qr && !qrGenerated) {
                    qrGenerated = true;

                    // Clear timeout since QR was generated
                    if (timeoutId) {
                        clearTimeout(timeoutId);
                        timeoutId = null;
                    }

                    try {
                        const qrDataURL = await QRCode.toDataURL(qr, {
                            errorCorrectionLevel: 'H',
                            width: 400
                        });

                        if (!responseSent) {
                            responseSent = true;
                            res.json({
                                qr: qrDataURL,
                                message: 'QR Code Generated! Scan with WhatsApp',
                                sessionId,
                                instructions: [
                                    '1. Open WhatsApp on your phone',
                                    '2. Go to Settings > Linked Devices',
                                    '3. Tap "Link a Device"',
                                    '4. Scan the QR code above'
                                ]
                            });
                        }
                    } catch (err) {
                        console.error(`[${sessionId}] QR generation error:`, err);
                        if (!responseSent) {
                            responseSent = true;
                            res.status(500).json({ error: 'QR generation failed' });
                        }
                    }
                }

                // Connection opened successfully
                if (connection === 'open') {
                    console.log(`[${sessionId}] ✅ Connected to WhatsApp!`);

                    // Clear timeout on successful connection
                    if (timeoutId) {
                        clearTimeout(timeoutId);
                        timeoutId = null;
                    }

                    if (!credsSent) {
                        credsSent = true;

                        try {
                            await delay(2000);

                            const credsFile = path.join(dirs, 'creds.json');

                            if (!fs.existsSync(credsFile)) {
                                console.error(`[${sessionId}] ❌ Creds file not found!`);
                                return;
                            }

                            const credsContent = await fs.readJson(credsFile);
                            if (!credsContent.me || !credsContent.me.id) {
                                console.error(`[${sessionId}] ❌ Invalid creds file`);
                                await delay(3000);
                                return;
                            }

                            console.log(`[${sessionId}] Creds validated. User: ${credsContent.me.id}`);

                            const uniqueCredsFile = path.join(dirs, `${sessionId}.json`);
                            fs.copySync(credsFile, uniqueCredsFile);

                            const userJid = credsContent.me.id;

                            console.log(`[${sessionId}] Sending session ID to user...`);
                            await sock.sendMessage(userJid, {
                                text: `🎉 *SESSION CREATED*\n\nSession ID: \`${sessionId}\`\n\nKeep this ID safe!`
                            });

                            await delay(2000);

                            console.log(`[${sessionId}] Sending creds file...`);
                            await sock.sendMessage(userJid, {
                                document: { url: uniqueCredsFile },
                                mimetype: 'application/json',
                                fileName: 'creds.json',
                                caption: MESSAGE
                            });

                            console.log(`[${sessionId}] ✅ Session file sent successfully!`);

                            await delay(3000);

                            sock.end(undefined);
                            console.log(`[${sessionId}] Socket closed. Use /load to reconnect.`);

                            // Background MEGA upload
                            (async () => {
                                try {
                                    await delay(2000);
                                    console.log(`[${sessionId}] Uploading to MEGA...`);
                                    const megaUrl = await upload(
                                        fs.createReadStream(credsFile),
                                        `${sessionId}.json`
                                    );
                                    if (megaUrl) {
                                        console.log(`[${sessionId}] 📄 MEGA: ${megaUrl}`);
                                        const metaFile = path.join(dirs, 'meta.json');
                                        await fs.writeJson(metaFile, {
                                            sessionId,
                                            megaUrl,
                                            createdAt: new Date().toISOString()
                                        });
                                    }
                                } catch (e) {
                                    console.error(`[${sessionId}] MEGA error:`, e.message);
                                }
                            })();

                        } catch (err) {
                            console.error(`[${sessionId}] Error sending session:`, err);
                        }
                    }
                }

                // Handle disconnection
                if (connection === 'close') {
                    const statusCode = lastDisconnect?.error?.output?.statusCode;
                    const reason = lastDisconnect?.error?.output?.payload?.error;

                    console.log(`[${sessionId}] Disconnected. Code: ${statusCode}, Reason: ${reason}`);

                    // If creds already sent, we're done
                    if (credsSent) {
                        console.log(`[${sessionId}] Session complete. No reconnection needed.`);
                        return;
                    }

                    // Handle specific disconnect reasons
                    if (statusCode === DisconnectReason.badSession) {
                        console.log(`[${sessionId}] ❌ Bad session. Cleaning up...`);
                        await fs.remove(dirs);
                        if (!responseSent) {
                            responseSent = true;
                            res.status(400).json({
                                error: 'Bad session',
                                message: 'Session corrupted. Please try again.'
                            });
                        }
                        return;
                    }

                    if (statusCode === DisconnectReason.loggedOut) {
                        console.log(`[${sessionId}] ❌ Logged out.`);
                        await fs.remove(dirs);
                        return;
                    }

                    if (statusCode === DisconnectReason.restartRequired) {
                        console.log(`[${sessionId}] Restart required...`);
                        setTimeout(() => initiateSession(), 2000);
                        return;
                    }

                    if (statusCode === DisconnectReason.timedOut) {
                        console.log(`[${sessionId}] ⏱️  Connection timed out`);
                        if (!qrGenerated && !responseSent) {
                            responseSent = true;
                            res.status(408).json({ error: 'Connection timeout' });
                        }
                        return;
                    }

                    // Unknown error - retry if still generating
                    if (!qrGenerated) {
                        console.log(`[${sessionId}] Retrying connection...`);
                        setTimeout(() => initiateSession(), 3000);
                    }
                }
            });

            // Set timeout - will be cleared if QR is generated or connection opens
            timeoutId = setTimeout(() => {
                if (!responseSent && !qrGenerated) {
                    console.log(`[${sessionId}] ⏱️  Timeout - no QR generated`);
                    responseSent = true;
                    res.status(408).json({ error: 'QR generation timeout' });
                    sock.end(undefined);
                }
            }, 60000);

        } catch (err) {
            console.error(`[${sessionId}] Fatal error:`, err);
            if (!res.headersSent) {
                res.status(500).json({ error: 'Session initialization failed' });
            }
        }
    }

    await initiateSession();
});

// ============================================
// LOAD EXISTING SESSION ENDPOINT
// ============================================
router.get('/load', async (req, res) => {
    const sessionId = req.query.sessionId;

    if (!sessionId) {
        return res.status(400).json({
            error: 'Session ID required',
            usage: '/load?sessionId=pgwiz-xxx'
        });
    }

    const dirs = path.join(SESSIONS_ROOT, sessionId);
    const credsPath = path.join(dirs, 'creds.json');

    // Check if session exists
    if (!fs.existsSync(credsPath)) {
        return res.status(404).json({
            error: 'Session not found',
            message: `No session found for ID: ${sessionId}`,
            sessionId
        });
    }

    // Check if already loaded
    if (activeConnections.has(sessionId)) {
        return res.json({
            message: 'Session already active',
            sessionId,
            status: 'connected'
        });
    }

    console.log(`[${sessionId}] Loading existing session...`);

    try {
        const { state, saveCreds } = await useMultiFileAuthState(dirs);
        const { version } = await fetchLatestBaileysVersion();

        const sock = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: false,
            browser: Browsers.ubuntu('Chrome'),
            auth: state,
            syncFullHistory: false,
            markOnlineOnConnect: true, // Show online when loaded
        });

        sock.ev.on('creds.update', saveCreds);

        sock.ev.on('connection.update', async (update) => {
            const { connection, lastDisconnect } = update;

            console.log(`[${sessionId}] LOAD - Connection: ${connection}`);

            if (connection === 'open') {
                console.log(`[${sessionId}] ✅ Session loaded successfully!`);

                // Store active connection
                activeConnections.set(sessionId, {
                    sock,
                    connectedAt: new Date().toISOString()
                });

                // Send confirmation message
                try {
                    const credsContent = await fs.readJson(credsPath);
                    const userJid = credsContent.me.id;

                    await sock.sendMessage(userJid, {
                        text: `✅ *SESSION LOADED*\n\nSession \`${sessionId}\` is now active!`
                    });
                } catch (e) {
                    console.error(`[${sessionId}] Could not send confirmation:`, e.message);
                }

                if (!res.headersSent) {
                    res.json({
                        success: true,
                        message: 'Session loaded successfully',
                        sessionId,
                        status: 'connected'
                    });
                }
            }

            if (connection === 'close') {
                const statusCode = lastDisconnect?.error?.output?.statusCode;
                console.log(`[${sessionId}] LOAD - Disconnected. Code: ${statusCode}`);

                // Remove from active connections
                activeConnections.delete(sessionId);

                if (statusCode === DisconnectReason.loggedOut) {
                    console.log(`[${sessionId}] ❌ Session logged out. Cleaning up...`);
                    await fs.remove(dirs);

                    if (!res.headersSent) {
                        res.status(401).json({
                            error: 'Session logged out',
                            message: 'Session was logged out from WhatsApp. Generate a new QR code.'
                        });
                    }
                    return;
                }

                // Auto-reconnect for other errors
                if (statusCode !== DisconnectReason.loggedOut &&
                    statusCode !== DisconnectReason.badSession) {
                    console.log(`[${sessionId}] Reconnecting in 3s...`);
                    setTimeout(() => {
                        if (!activeConnections.has(sessionId)) {
                            // Recursive reload
                            req.query.sessionId = sessionId;
                            router.handle(req, { ...res, headersSent: true }, () => { });
                        }
                    }, 3000);
                }
            }
        });

        // Timeout for load
        setTimeout(() => {
            if (!res.headersSent) {
                res.status(408).json({
                    error: 'Connection timeout',
                    message: 'Could not connect to WhatsApp. Session may be expired.'
                });
                sock.end(undefined);
            }
        }, 45000);

    } catch (err) {
        console.error(`[${sessionId}] Load error:`, err);
        if (!res.headersSent) {
            res.status(500).json({
                error: 'Failed to load session',
                message: err.message
            });
        }
    }
});

// ============================================
// DISCONNECT SESSION ENDPOINT
// ============================================
router.post('/disconnect/:sessionId', async (req, res) => {
    const { sessionId } = req.params;
    const connection = activeConnections.get(sessionId);

    if (!connection) {
        return res.status(404).json({
            error: 'Session not found',
            message: 'No active session with this ID'
        });
    }

    try {
        await connection.sock.end(undefined);
        activeConnections.delete(sessionId);
        console.log(`[${sessionId}] Session disconnected manually`);

        res.json({
            success: true,
            message: 'Session disconnected',
            sessionId
        });
    } catch (err) {
        res.status(500).json({
            error: 'Failed to disconnect',
            message: err.message
        });
    }
});

// ============================================
// GET ACTIVE SESSIONS
// ============================================
router.get('/active', (req, res) => {
    const sessions = [];
    for (const [sessionId, data] of activeConnections) {
        sessions.push({
            sessionId,
            connectedAt: data.connectedAt
        });
    }

    res.json({
        count: sessions.length,
        sessions
    });
});

// ============================================
// LIST ALL SAVED SESSIONS
// ============================================
router.get('/sessions', async (req, res) => {
    try {
        if (!fs.existsSync(SESSIONS_ROOT)) {
            return res.json({ sessions: [] });
        }

        const folders = await fs.readdir(SESSIONS_ROOT);
        const sessions = [];

        for (const folder of folders) {
            const sessionPath = path.join(SESSIONS_ROOT, folder);
            const credsPath = path.join(sessionPath, 'creds.json');

            if (fs.existsSync(credsPath)) {
                const stats = await fs.stat(credsPath);
                const metaPath = path.join(sessionPath, 'meta.json');
                let meta = null;

                if (fs.existsSync(metaPath)) {
                    meta = await fs.readJson(metaPath);
                }

                sessions.push({
                    sessionId: folder,
                    createdAt: meta?.createdAt || stats.birthtime.toISOString(),
                    megaUrl: meta?.megaUrl || null,
                    isActive: activeConnections.has(folder)
                });
            }
        }

        res.json({
            count: sessions.length,
            sessions
        });
    } catch (err) {
        res.status(500).json({
            error: 'Failed to list sessions',
            message: err.message
        });
    }
});

// Error handling
process.on('uncaughtException', (err) => {
    const ignore = [
        "conflict", "not-authorized", "Connection Closed",
        "Timed Out", "Stream Errored", "code 1006",
        "ERR_HTTP_HEADERS_SENT"
    ];

    if (!ignore.some(x => String(err).includes(x))) {
        console.error('Uncaught exception:', err);
    }
});

export default router;